...
@interface YMPhotosView ()<UIScrollViewDelegate,UIGestureRecognizerDelegate>
...
- (void)longPress:(UILongPressGestureRecognizer *)press{
    ...
    if (press.state == UIGestureRecognizerStateBegan) {
        ...
        YMActionSheetView *acitionSheet = [[YMActionSheetView alloc]initWithTitle:@"" buttons:@[@"保存图片",@"取消"] buttonClick:^(YMActionSheetView *sheetView, NSInteger buttonIndex) {
            SQPhotoItemScrollView *itemView = self.itemViews[_currentPage];
            
            
            if (buttonIndex == 0) {
                [self savePhotoWithImage:itemView.imageView.image completion:^{
                    [LCProgressHUD showMessage:@"保存成功"];
                }];
            }
